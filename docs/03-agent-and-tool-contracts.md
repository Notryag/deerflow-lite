...
- legacy `research_agent`、`writer_agent` 和 `subagent runner` MAY 复用同一组 helper
...
research notes 和 final report 的产出 SHOULD 通过 tool / helper 层完成，而不是依赖固定 `ResearchAgent` / `WriterAgent` 节点。
...
- 生成 notes 和 final report 的逻辑 SHOULD 可被 workflow、legacy agent 和 subagent runtime 复用
